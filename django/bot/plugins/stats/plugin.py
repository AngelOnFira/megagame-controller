...
class Plugin(BasePlugin):
    ...
    async def on_message(self, message):
        ...
        member = await Member.objects.from_message(message)
        channel = await Channel.objects.from_message(message)

        n_lines = len(message.content.splitlines())

        logged_message = await sync_to_async(LoggedMessage.objects.create)(
        ...
        if message.mentions:
            logged_message.mentions.set(
                await Member.objects.from_mentions(message.mentions)
            )
    ...
    async def stat_messages(self, command):
        ...
        queryset = Member.objects.exclude(is_bot=True).annotate(
            num_messages=Count(
                "messages_authored", filter=Q(messages_authored__server=server_id)
            )
        )
    ...
    async def seen(self, command):
        if command.message.mentions:
            ...
            d_member = next(
                (m for m in all_members if m.name.lower() == username.lower()), None
            )
